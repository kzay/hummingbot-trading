...
#
# Requires: env/.env exists (copy from env/.env.template)
# ============================================
...
COMPOSE_DIR="$HB_ROOT/compose"
ENV_FILE="$HB_ROOT/env/.env"

if [[ ! -f "$ENV_FILE" ]]; then
  echo "[compose_up] ERROR: env file not found: $ENV_FILE"
  echo "[compose_up] Copy from template: cp $HB_ROOT/env/.env.template $ENV_FILE"
  exit 1
fi
