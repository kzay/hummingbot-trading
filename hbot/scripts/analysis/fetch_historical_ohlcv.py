...
Usage:
    ...
    python hbot/scripts/analysis/fetch_historical_ohlcv.py --months 3 --output hbot/data/historical
...
def _fetch_ohlcv_ccxt(
    ...
    while current_since < until_ms:
        ...
        print(f"  Fetched {len(all_bars)} bars so far (last: {datetime.fromtimestamp(last_ts / 1000, tz=timezone.utc).isoformat()})  ", end="\r")
        time.sleep(delay_s)
...
def fetch_months(
    ...
    timeframe: str = "1m",
    output_dir: str = "hbot/data/historical",
) -> List[Path]:
...
def merge_parquets(paths: List[Path], output_path: Optional[Path] = None) -> Path:
...
if __name__ == "__main__":
    ...
    ap.add_argument("--output", default="hbot/data/historical")
